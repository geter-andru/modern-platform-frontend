...
async function seedMVPData() {
    try {
        ...
        // Step 1: Note: Using upsert approach for idempotent seeding
        console.log('🔄 Using upsert approach for idempotent seeding...');
        console.log('   (No data clearing needed - will update existing records)');

        console.log('');

        // Step 2: Upsert Resources
        console.log('📚 Upserting 4 MVP resources...');
        
        const { data: insertedResources, error: resourcesError } = await supabase
            .from('resources')
            .upsert(mvpResources, { onConflict: 'customer_id,title' })
        ...
        console.log(`✅ Successfully upserted ${insertedResources.length} resources:`);
        ...
        // Step 4: Upsert Customer Assets
        console.log('👥 Upserting customer assets for Brandon and Dotun...');
        
        const { data: insertedCustomers, error: customersError } = await supabase
            .from('customer_assets')
            .upsert([brandonData, dotunData], { onConflict: 'email' })
        ...
        console.log(`✅ Successfully upserted ${insertedCustomers.length} customer records:`);
