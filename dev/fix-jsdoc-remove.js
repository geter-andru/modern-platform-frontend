...
/**
 * Remove problematic JSDoc comments and replace with simple comments
 */
...
function fixJSDocComments(content) {
  ...
  // Remove all problematic JSDoc patterns and replace with simple comments
  // Pattern 1: Remove malformed JSDoc blocks
  fixed = fixed.replace(/\*\*\s*\n\s*\*\*\s*\n/g, '// ');
  
  // Pattern 2: Remove standalone * characters
  fixed = fixed.replace(/^\s*\*\s*$/gm, '// ');
  
  // Pattern 3: Remove malformed JSDoc blocks with multiple /** patterns
  fixed = fixed.replace(/\*\*\s*\n\s*\*\*\s*\n\s*\*/g, '// ');
  
  // Pattern 4: Remove incomplete JSDoc blocks
  fixed = fixed.replace(/\*\*\s*\n\s*\*\*\s*\n\s*\*\s*\n/g, '// ');
  
  // Pattern 5: Remove lines that start with * but aren't properly formatted
  fixed = fixed.replace(/^\s*\*\s*$/gm, '// ');
...
function main() {
  console.log('🔧 Starting JSDoc removal process...');
