...
const WeeklyTargetAccounts: React.FC<WeeklyTargetAccountsProps> = ({
  ...
  */
  ...
  useEffect(() => {
    const loadAccounts = async () => {
      ...
      try {
        ...
        // const response = await fetch(`/api/target-accounts/${customerId}`);
        // const data = await response.json();
        // setAccounts(data);
        
        // For now, set empty array - data will be loaded via API
        setAccounts([]);
      } catch (error) {
  ...
  // Check feature flag - show "Coming Soon" if disabled in production
  if (!isFeatureEnabled('weeklyTargetAccounts')) {
    return (
      <div className={`bg-gradient-to-r from-blue-900/20 to-purple-900/20 border border-blue-500/30 rounded-xl p-12 ${className}`}>
        <div className="text-center max-w-2xl mx-auto">
          ...
          <h3 className="text-2xl font-bold text-white mb-3">
            Target Account Intelligence Coming Soon
          </h3>

          <p className="text-gray-300 mb-4 text-lg">
            We're building AI-powered prospecting to automatically identify your best-fit accounts based on your ICP.
          </p>
          ...
          <div className="bg-gray-800/50 rounded-lg p-6 mb-6 text-left">
            ...
            <ul className="space-y-2 text-gray-400 text-sm">
              <li className="flex items-start">
                ...
                <span>Weekly AI-generated list of 20 companies matching your ICP</span>
              </li>
              <li className="flex items-start">
                <CheckCircle className="w-4 h-4 text-green-400 mr-2 mt-0.5 flex-shrink-0" />
                <span>Deep research on pain points, strategic initiatives, and key stakeholders</span>
              </li>
              <li className="flex items-start">
                <CheckCircle className="w-4 h-4 text-green-400 mr-2 mt-0.5 flex-shrink-0" />
                <span>Real-time news and trigger events for timely outreach</span>
              </li>
              <li className="flex items-start">
                <CheckCircle className="w-4 h-4 text-green-400 mr-2 mt-0.5 flex-shrink-0" />
                <span>LinkedIn intelligence on decision-makers and buying committees</span>
              </li>
            </ul>
          </div>
          ...
          <div className="inline-flex items-center space-x-3">
            <button
              ...
              onClick={() => window.open('https://forms.gle/your-waitlist-form', '_blank')}
            >
              Join Waitlist
            </button>
            <span className="text-gray-500 text-sm">Available Q1 2026</span>
          </div>

          <p className="text-gray-500 text-xs mt-6">
            This is a TIER 4 feature requiring Puppeteer MCP + LinkedIn MCP integration (~40-60 hours development)
          </p>
        </div>
      </div>
    );
