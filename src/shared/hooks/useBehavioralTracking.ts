...
export const useBehavioralTracking = (
  ...
  const behavioralService = useRef<any | null>(null);

  // Initialize behavioral service
  useEffect(() => {
    if (typeof window !== 'undefined') {
      // behavioralService.current = new (BehavioralIntelligenceService as any)();
      behavioralService.current = {}; // Placeholder for behavioral service
    }
  }, []);
