...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      try {
        const authServiceModule = await import('../../../lib/services/authService' as any).catch(() => ({}));
        const airtableServiceModule = await import('../../../lib/services/airtableService' as any).catch(() => ({}));
        const authService = authServiceModule.authService || {};
        const airtableService = airtableServiceModule.airtableService || {};
  ...
  useEffect(() => {
    const loadLoadingSpinner = async () => {
      try {
        const module = await import('../common/LoadingSpinner' as any).catch(() => ({}));
  ...
  return (
    <div className="min-h-screen bg-primary">
      ...
      {customerData?.isAdmin && (
        <div>{/* <AdminModeIndicator isVisible={true} position="top-right" /> */}</div>
      )}
