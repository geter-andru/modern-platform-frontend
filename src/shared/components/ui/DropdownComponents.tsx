        ...
        {React.Children.map(children, (child) => {
          if (React.isValidElement(child) && child.type === MultiSelectOption) {
            return React.cloneElement(child as React.ReactElement<MultiSelectOptionProps>, {
              ...
              isSelected: values.includes((child.props as any).value),
              disabled: (child.props as any).disabled || (maxSelections && values.length >= maxSelections && !values.includes((child.props as any).value))
            });
          }
          return child;
        })}
