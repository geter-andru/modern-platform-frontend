...
/**
 * RecentMilestones - Recent achievements and milestones widget
 *
 * Features:
 * - Displays recent 3 milestones with details
 * - Professional milestone categories (6 types)
 * - Level-based styling (foundation to expert)
 * - Recent milestone animation (pulse effect)
 * - "View All" navigation for full history
 * - Empty state with encouragement
 * - Summary statistics (total achieved, this week)
 * - Professional encouragement messaging
 * - Responsive design
 */
...
export interface Milestone {
  ...
  description: string;
  /** Completion timestamp display (e.g., "2 hours ago") */
  completedAt: string;
  /** Milestone category */
...
export interface MilestoneItemProps {
  ...
  /** Achievement level */
  level: MilestoneLevel;
  /** Whether this is a very recent milestone (shows animation) */
  isRecent?: boolean;
  /** Additional CSS classes */
  className?: string;
}

export interface RecentMilestonesProps {
  /** Array of milestones (shows most recent 3) */
  milestones?: Milestone[];
  /** Callback to show all milestones */
  showAll?: () => void;
  /** Additional CSS classes */
  className?: string;
}
 ...
 */
export const MILESTONE_CATEGORIES: Record<MilestoneCategory, MilestoneCategoryConfig> = {
  customerAnalysis: {
    icon: Target,
 ...
 */
export const LEVEL_STYLES: Record<MilestoneLevel, LevelStyleConfig> = {
  foundation: {
...
/**
 * Determine if milestone is very recent (for animation)
 */
function isRecentMilestone(milestone: Milestone): boolean {
  const completedText = milestone.completedAt?.toLowerCase() || '';
  return (
    completedText.includes('hour') ||
    completedText.includes('minute') ||
    completedText === 'today'
  );
}
...
export const RecentMilestones: React.FC<RecentMilestonesProps> = ({
  ...
  // Count recent milestones (this week)
  const recentCount = milestones.filter(isRecentMilestone).length;
      ...
      {recentMilestones.length > 0 ? (
        ...
        <div className="text-center py-6 px-4 border-2 border-dashed border-purple-700/50 rounded-lg">
          ...
          <p className="text-purple-300 text-sm leading-relaxed">
            Complete professional development activities to earn recognition for your systematic
            advancement
          </p>
      ...
      {recentMilestones.length > 0 && (
        <div className="mt-4 p-3 bg-black/30 border border-purple-700/30 rounded-lg">
          <div className="grid grid-cols-2 gap-4 text-center">
            <div>
              <div className="text-lg font-bold text-white">{milestones.length}</div>
              <div className="text-xs text-purple-200 uppercase tracking-wide">Total Achieved</div>
            </div>
            <div>
              <div className="text-lg font-bold text-amber-400">{recentCount}</div>
              <div className="text-xs text-purple-200 uppercase tracking-wide">This Week</div>
            </div>
          </div>
