...
/**
 * QuickActionButton - Professional action button component
 *
 * Features:
 * - Enabled/disabled states with lock indicators
 * - Progress bars for unlock tracking
 * - Category-based color schemes
 * - Estimated time and point values
 * - Icon support with mapping
...
export interface QuickActionButtonProps {
  ...
  /** Click handler */
  onClick: () => void;
  /** Whether action is enabled/unlocked */
  enabled?: boolean;
  /** Category for color scheme */
...
const ACTION_ICONS: Record<ActionIconType, LucideIcon> = {
  'user-plus': UserPlus,
...
const CATEGORY_STYLES: Record<ActionCategory, CategoryStyles> = {
  customerAnalysis: {
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between text-xs">
          ...
          {pointValue && (
            <div
              className={`flex items-center space-x-1 ${
                enabled ? 'text-green-400' : 'text-gray-600'
              }`}
            >
              <TrendingUp size={12} />
        ...
        {/* Requirement Text */}
        {!enabled && requirement && (
          <div className="text-xs text-gray-500 italic">Requires: {requirement}</div>
        )}
      </div>
