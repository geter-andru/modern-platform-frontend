...
export default function TaskProgress({
  ...
  const milestoneData = useMemo(() => {
    const targets = TaskRecommendationEngine.getMilestoneTargets(milestone.tier);
  ...
  const nextMilestoneData = useMemo(() => {
    const nextMilestone = TaskRecommendationEngine.getNextMilestone(milestone);
    if (!nextMilestone) return null;

    const nextTargets = TaskRecommendationEngine.getMilestoneTargets(nextMilestone.tier);
    const avgCurrent =
      (competencyScores.customerAnalysis +
        competencyScores.valueCommunication +
        competencyScores.executiveReadiness) /
