...
export default function TaskDetails({
  ...
  useEffect(() => {
    if (task && isOpen) {
      const customerData = {
        customerId: 'current-user', // Would come from props
        competencyScores,
        milestone
      };
  ...
  const competencyDisplay = getCompetencyDisplay(task.competencyArea);
  const competencyGain = getCompetencyGain(task.priority);
  const currentCompetency = competencyScores[task.competencyArea] || 0;
