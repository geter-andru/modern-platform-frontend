...
/**
 * ModernModal - Flexible modal dialog system for Next.js App Router
 *
 * Features:
 * - Multiple modal sizes (sm, md, lg, xl, 2xl, full)
 * - Multiple variants (default, centered, drawer, fullscreen)
 * - Animated entrance/exit transitions with spring physics
 * - Backdrop blur and click-outside-to-close
 * - Full keyboard navigation (ESC to close, Tab for focus trap)
 * - Full-screen mode support with toggle button
 * - Custom header and footer sections
 * - Scroll handling for large content (max 80vh)
 * - Focus management and accessibility (restores previous focus on close)
 * - Body scroll prevention when open
 * - ConfirmationModal component for confirmations
 * - AlertModal component for alerts
              ...
              {(title || header || showCloseButton || showFullscreenToggle) && (
                <div className="flex items-center justify-between p-6 border-b border-gray-700">
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    {title && (
                      <h2 id="modal-title" className="text-xl font-semibold text-white truncate">
                        {title}
                      </h2>
                    )}
                    {header}
...
export const ModernConfirmationModal: React.FC<ConfirmationModalProps> = ({
  isOpen,
  onClose,
  onConfirm,
  title,
...
export const ModernAlertModal: React.FC<AlertModalProps> = ({
  isOpen,
  onClose,
  title,
  message,
