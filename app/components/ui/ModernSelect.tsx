...
/**
 * ModernSelect - Advanced select dropdown component for Next.js App Router
 *
 * Features:
 * - Single and multi-select modes
 * - Search/filter functionality with auto-focus
...
const ModernSelect = forwardRef<HTMLDivElement, SelectProps>(({
  options,
  value,
  defaultValue,
  onChange,
            ...
            {multiple && selectedOptions.length > 0 ? (
              <div className="flex items-center gap-1 flex-wrap">
                {selectedOptions.slice(0, 2).map(option => (
                  <span
                    key={option.value}
                    className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-500/20 text-purple-400 text-xs rounded-md"
                  >
                    {option.label}
                    <button
                      onClick={(e) => {
