...
/**
 * POST /api/export/company-rating
 *
 * Proxies company rating export requests to the Express backend
...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendResponse = await fetch(getBackendUrl('/api/export/company-rating'), {
      method: 'POST',
      headers: {
    ...
    if (!backendResponse.ok) {
      return NextResponse.json(
        { success: false, error: result.error || 'Failed to export company rating data' },
    ...
    console.error('Error in export/company-rating API route:', error);
    return NextResponse.json(
