 ...
 *
 * Proxies business case export requests to the Express backend
...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendResponse = await fetch(getBackendUrl('/api/export/business-case'), {
      method: 'POST',
      headers: {
    ...
    if (!backendResponse.ok) {
      return NextResponse.json(
        { success: false, error: result.error || 'Failed to export business case data' },
    ...
    console.error('Error in export/business-case API route:', error);
    return NextResponse.json(
