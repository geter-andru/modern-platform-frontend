...
/**
 * POST /api/export/personas
 *
 * Proxies personas export requests to the Express backend
...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendResponse = await fetch(getBackendUrl('/api/export/personas'), {
      method: 'POST',
      headers: {
    ...
    if (!backendResponse.ok) {
      return NextResponse.json(
        { success: false, error: result.error || 'Failed to export personas data' },
    ...
    console.error('Error in export/personas API route:', error);
    return NextResponse.json(
