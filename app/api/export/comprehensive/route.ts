 ...
 *
 * Proxies comprehensive export requests to the Express backend
...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendResponse = await fetch(getBackendUrl('/api/export/comprehensive'), {
      method: 'POST',
      headers: {
    ...
    if (!backendResponse.ok) {
      return NextResponse.json(
        { success: false, error: result.error || 'Failed to export comprehensive data' },
    ...
    console.error('Error in export/comprehensive API route:', error);
    return NextResponse.json(
