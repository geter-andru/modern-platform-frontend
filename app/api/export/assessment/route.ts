...
/**
 * POST /api/export/assessment
 *
 * Proxies assessment export requests to the Express backend
...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendResponse = await fetch(getBackendUrl('/api/export/assessment'), {
      method: 'POST',
      headers: {
    ...
    if (!backendResponse.ok) {
      return NextResponse.json(
        { success: false, error: result.error || 'Failed to export assessment data' },
    ...
    console.error('Error in export/assessment API route:', error);
    return NextResponse.json(
