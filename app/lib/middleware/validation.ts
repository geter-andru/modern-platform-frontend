...
export const apiSchemas = {
  ...
  assessment: {
    create: z.object({
      ...
      answers: z.record(z.string(), z.any()).refine((val) => Object.keys(val).length >= 1, 'Assessment answers are required'),
      metadata: z.object({
  ...
  export: {
    request: z.object({
      ...
      data: z.record(z.string(), z.any()).refine((val) => Object.keys(val).length >= 1, 'Export data is required'),
      options: z.object({
  ...
  agent: {
    execute: z.object({
      ...
      input: z.record(z.string(), z.any()).refine((val) => Object.keys(val).length >= 1, 'Agent input is required'),
      options: z.object({
...
// Export types (ValidationErrorResponse already exported above)
