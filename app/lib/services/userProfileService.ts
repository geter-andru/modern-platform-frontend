...
import { ROLES } from '@/app/lib/middleware/rbac';
...
export class UserProfileService {
  ...
  static async getAllProfiles(limit: number = 100, offset: number = 0): Promise<UserProfile[]> {
    ...
    return profiles.map((profile: any) => this.transformProfile(profile));
  }
  ...
  static async getUserStatistics(): Promise<{
    ...
    // Get total users
    const { count: totalUsers, error: totalError } = await (supabase as any)
      .from('user_profiles')
    ...
    const usersBySubscription = subData.reduce((acc: any, user: any) => {
    ...
    const { count: newUsersThisMonth, error: newUsersError } = await (supabase as any)
      .from('user_profiles')
      .select('*', { count: 'exact', head: true })
