...
class ResourceExportService implements IResourceExportService {
  ...
  private async generatePDF(resource: Resource, options?: Record<string, any>): Promise<Buffer> {
    ...
    const sections = (resource.content?.sections || []).map((section: any) => ({
      title: section.title || '',
    ...
    const pdfOptions = {
      ...
      subtitle: resource.description || 'Resource Export',
    ...
    const blob = await PDFGenerator.generateFromSections(sections, pdfOptions);
  ...
  private async generateDOCX(resource: Resource, options?: Record<string, any>): Promise<Buffer> {
    // Prepare sections for DOCX generation
    const sections = (resource.content?.sections || []).map((section: any) => ({
      title: section.title || '',
      content: section.content || '',
      type: section.type || 'text'
    ...
    // Generate DOCX using the shared generator
    const blob = await DOCXGenerator.generateFromSections(sections, docxOptions);
