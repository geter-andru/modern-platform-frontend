...
class ExportService {
  ...
  private async generatePDF(request: ExportRequest): Promise<Blob> {
    ...
    switch (type) {
      case 'icp':
        return await PDFGenerator.generateICPAnalysis(data, pdfOptions);
      case 'assessment':
        return await PDFGenerator.generateAssessment(data, pdfOptions);
      case 'business_case':
        return await PDFGenerator.generateBusinessCase(data, pdfOptions);
      case 'cost':
        return await PDFGenerator.generateCostCalculator(data, pdfOptions);
      case 'comprehensive':
        // Generate comprehensive report with all sections
        return await PDFGenerator.generateFromSections(data.sections || [], pdfOptions);
      default:
