...
export class ConfigurationTester {
  ...
  private async testStripeConfiguration(): Promise<void> {
    try {
      ...
      const isValid = (stripe as any).validateStripeKey(config.publishableKey, 'stripe');
      this.addTestResult(
  ...
  private async testErrorHandling(): Promise<void> {
    try {
      ...
      this.addTestResult(
        ...
        { error: { type: error.type, message: error instanceof Error ? error.message : String(error) } }
      );
