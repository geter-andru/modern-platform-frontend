  ...
  private async loadAgents(): Promise<void> {
    ...
    try {
      ...
      if (this.config.enableManual) {
        this.agents.manual = new (ManualAgent as any)(
          this.supabase,
          this.agents.audit,
          this.agents.optimization,
          this.agents.backup,
    ...
    try {
      ...
      switch (operation) {
        ...
        case 'ad_hoc_task':
          ...
          result = await (this.agents.manual as any).execute(operation, event);
          agentType = 'manual';
          break;
  ...
  public getStatus(): OrchestratorStatus {
    return {
      ...
      agents: {
        ...
        manual: (this.agents.manual?.getStatus() as any) || { isActive: false, isReady: false, lastActivity: null, performance: {}, errorCount: 0, successCount: 0 }
      },
